#include "nonlinear_our.h"

NonLinear_Our::NonLinear_Our(int party, string address, int port){
    this->party = party;
...
void softmax_thread_our(int tid, int party, uint64_t *x, uint64_t *y, uint64_t* l, int num_ops, int array_size, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
  ...
  FixArray l_short;
  tie(output_array, l_short) = fpmath->softmax_fix_our(input_array);
  for(int i = 0; i < num_ops; i++){
...
void NonLinear_Our::softmax(int nthreads, uint64_t* input, uint64_t* output, uint64_t* l, int dim, int array_size, int ell, int s){
...
void softmax_irons_thread_our(int tid, int party, uint64_t *x, uint64_t *y, int num_ops, int array_size, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::softmax_iron(int nthreads, uint64_t* input, uint64_t* output, int dim, int array_size, int ell, int s){
    std::thread threads[nthreads];
    int chunk_size = dim / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void layer_norm_thread_our(int tid, int party, uint64_t *x, uint64_t *y, uint64_t *w, uint64_t *b, int num_ops, int array_size, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::layer_norm(int nthreads, uint64_t* input, uint64_t* output, uint64_t* weight, uint64_t* bias, int dim, int array_size, int ell, int s){
    std::thread threads[nthreads];
    int chunk_size = dim / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void gelu_thread_our(int tid, int party, uint64_t *x, uint64_t *y, int num_ops, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::gelu(int nthreads, uint64_t* input, uint64_t* output, int size, int ell, int s){
    std::thread threads[nthreads];
...
void gelu_iron_thread_our(int tid, int party, uint64_t *x, uint64_t *y, int num_ops, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::gelu_iron(int nthreads, uint64_t* input, uint64_t* output, int size, int ell, int s){
    std::thread threads[nthreads];
    int chunk_size = size / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void tanh_thread_our(int tid, int party, uint64_t *x, uint64_t *y, int num_ops, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::tanh(int nthreads, uint64_t* input, uint64_t* output, int size, int ell, int s){
    std::thread threads[nthreads];
    int chunk_size = size / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void tanh_iron_thread_our(int tid, int party, uint64_t *x, uint64_t *y, int num_ops, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::tanh_iron(int nthreads, uint64_t* input, uint64_t* output, int size, int ell, int s){
    std::thread threads[nthreads];
    int chunk_size = size / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void gt_p_sub_thread_our(int tid, int party, uint64_t *x, uint64_t p, uint64_t *y, int num_ops, int ell, int s_in, int s_out, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::gt_p_sub(int nthreads, uint64_t* input, uint64_t p, uint64_t* output, int size, int ell, int s_in, int s_out){
    std::thread threads[nthreads];
    int chunk_size = size / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void mul_thread_our(int tid, int party, uint64_t *x, uint64_t* z, uint64_t *y, int num_ops, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void  NonLinear_Our::n_matrix_mul(
  int nthreads,
  uint64_t* input_1,
  uint64_t* input_2,
  uint64_t* output,
...
void  NonLinear_Our::n_matrix_mul_iron(
  int nthreads,
  uint64_t* input_1,
  uint64_t* input_2,
  uint64_t* output,
...
void  NonLinear_Our::p_matrix_mul_iron(
  int nthreads,
  uint64_t* input_1,
  uint64_t* input_2,
  uint64_t* output,
...
void right_shift_thread_our(int tid, int party, uint64_t *x, int a, uint64_t *y, int num_ops, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::right_shift(int nthreads, uint64_t* input, int a, uint64_t* output, int size, int ell, int s){
    std::thread threads[nthreads];
    int chunk_size = size / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void reduce_thread_our(int tid, int party, uint64_t *x, uint64_t *y, int num_ops, int ell_in, int ell_out, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::reduce(int nthreads, uint64_t* input, uint64_t* output, int size, int ell_in, int ell_out, int s){
  std::thread threads[nthreads];
    int chunk_size = size / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void NonLinear_Our::print_ss(uint64_t* input, int length, int ell, int s){
  FixArray tmp = fpmath[0]->fix->input(party, length, input, true, ell, s);
  fpmath[0]->print(tmp);
}

 FixArray NonLinear_Our::to_public(uint64_t* input, int length, int ell, int s){
  FixArray tmp = fpmath[0]->fix->input(party, length, input, true, ell, s);
...
void cancel_wrap_thread_our(int tid, int party, uint64_t *x, uint64_t *y, int num_ops, int ell, int s, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::cancel_wrap(int nthreads, uint64_t* input, uint64_t* output, int size, int ell, int s){
  std::thread threads[nthreads];
    int chunk_size = size / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
...
void convert_l_to_p_thread_our(int tid, int party, uint64_t *x, uint64_t *y, int num_ops, int ell, int s, int l, uint64_t p, FPMath *fpmath) {
  int this_party;
  if (tid & 1) {
    this_party = 3 - party;
  } else {
...
void NonLinear_Our::convert_l_to_p(int nthreads, uint64_t* input, uint64_t* output, int l, uint64_t p, int size, int ell, int s){
  std::thread threads[nthreads];
    int chunk_size = size / nthreads;
    for (int i = 0; i < nthreads; ++i) {
        int offset = i * chunk_size;
