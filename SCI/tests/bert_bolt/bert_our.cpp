#include "bert_our.h"
// BERT_OUR: Using custom softmax implementation (softmax_fix_our)
...
inline uint64_t Bert_Our::get_comm(){
...
inline uint64_t Bert_Our::get_round(){
...
Bert_Our::Bert_Our(int party, int port, string address, string model_path, bool prune){
    this->party = party;
    ...
    this->nl = NonLinear_Our(party, address, port+1);
...
void Bert_Our::he_to_ss_server(HE* he, vector<Ciphertext> in, uint64_t* output, bool ring){
    #ifdef BERT_PERF
...
vector<Ciphertext> Bert_Our::ss_to_he_server(HE* he, uint64_t* input, int length, int bw){
    #ifdef BERT_PERF
    auto t_conversion = high_resolution_clock::now();
    #endif 
    int slot_count = he->poly_modulus_degree;
...
void Bert_Our::he_to_ss_client(HE* he, uint64_t* output, int length, const FCMetadata &data){
    #ifdef BERT_PERF
    auto t_conversion = high_resolution_clock::now();
    #endif 
...
void Bert_Our::ss_to_he_client(HE* he, uint64_t* input, int length, int bw){
    #ifdef BERT_PERF
    auto t_conversion = high_resolution_clock::now();
    #endif 
    int slot_count = he->poly_modulus_degree;
...
void Bert_Our::pc_bw_share_server(
    uint64_t* wp,
    uint64_t* bp,
    uint64_t* wc,
    uint64_t* bc
...
void Bert_Our::pc_bw_share_client(
    uint64_t* wp,
    uint64_t* bp,
    uint64_t* wc,
    uint64_t* bc
...
void Bert_Our::print_p_share(uint64_t* s, uint64_t p, int len){
    if(party == ALICE){
...
void Bert_Our::check_p_share(uint64_t* s, uint64_t p, int len, uint64_t* ref){
    if(party == ALICE){
        io->send_data(s, len*sizeof(uint64_t));
    } else{
        uint64_t* s1 = new uint64_t[len];
...
vector<double> Bert_Our::run(string input_fname, string mask_fname){
    // Server: Alice
    // Client: Bob
