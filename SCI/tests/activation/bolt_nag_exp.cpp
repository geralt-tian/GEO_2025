...
LinearOT *prod;
XTProtocol *ext;
// DReLUFieldProtocol *drelu_field_oracle;
...
int bwL_input = 13;
uint64_t mask_bwL_exp = (bwL_input == 64 ? -1 : ((1ULL << bwL_input) - 1));
uint64_t N_input = pow(2, bwL_input);
uint64_t f_input = 10;
uint64_t pow_f_input = pow(2, f_input);

int bwL_original = 15;
uint64_t mask_bwL_original = (bwL_original == 64 ? -1 : ((1ULL << bwL_original) - 1));
uint64_t N_original = pow(2, bwL_original);
...
//   drelu_field_oracle = new DReLUFieldProtocol(party, bwL_input, 4, 1000000001261, iopack, otpack);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    double exp_val = std::exp(-i * static_cast<double>(N_input) / pow_f_input) * std::exp(-4); // 直接在这里乘exp(-4)
  ...
  std::uniform_int_distribution<int> dis(0, (N_original/4) - 1); //0 - 16
//   std::uniform_int_distribution<int> dis((N_original/8) , (N_original/4) - 1);//0 - 8
  uint64_t *inA_original = new uint64_t[dim];
  uint64_t *inB_original = new uint64_t[dim];
  for (int i = 0; i < dim; i++) {
    inA_original[i] = dis(gen);
    inB_original[i] = dis(gen);
  }

  uint8_t *drelu = new uint8_t[dim];
  uint64_t *drelu_input_alice = new uint64_t[dim];
  uint64_t *drelu_input_bob = new uint64_t[dim];
  for (int i = 0; i < dim; i++) {
    drelu_input_alice[i] = (-inA_original[i] + 8*pow_f_input) & mask_bwL_original;
    drelu_input_bob[i] = (-inB_original[i] ) & mask_bwL_original;
  }
  if(party == ALICE){
    aux->MSB(drelu_input_alice, drelu, dim, bwL_original);
  } else {
    aux->MSB(drelu_input_bob, drelu, dim, bwL_original);
  }
  if(party == ALICE){
  for (int i = 0; i < dim; i++) {
    drelu[i] = 1- drelu[i];
...
//   for (int i = 0; i < dim; i++) {
//     printf("drelu_input_alice[%d]: %llu\n", i, drelu_input_alice[i]);
//     printf("drelu_input_bob[%d]: %llu\n", i, drelu_input_bob[i]);
//     printf("drelu[%d]: %d\n", i, drelu[i]);
//   }


  for (int i = 0; i < dim; i++) {
    inA[i] = (-inA_original[i] + 4*pow_f_input) & mask_bwL_exp;
    inB[i] = (-inB_original[i] ) & mask_bwL_exp;
  ...
  if (party == ALICE){
  gp->mwwithB(dim, MW_B, inA, MW, bwL_input, 2);
  }
  else{
    gp->mwwithB(dim, MW_B, inB, MW, bwL_input, 2);
  }

  if (party == ALICE){  //send MW to bob
...
// if (party != ALICE){
//   for (int i = 0; i < dim; i++) {
//     // printf("MW_plain[%d]: %llu\n", i, MW_plain[i]);
//     // printf("MW[%d]: %llu\n", i, MW[i]);
//     if (MW_plain[i] != MW[i]) {
//       printf("MW_plain[%d]: %llu\n", i, MW_plain[i]);
//       printf("MW[%d]: %llu\n", i, MW[i]);
//       printf("MW_plain[%d] != MW[%d]\n", i, i);
//     }
//   }
// }
...
// !!!!!!!!!!!!!!!!!!!!!!!TODO!!!!!!!!!!!!!!!!!!!!!!!!!
  if (party == ALICE) {
    gp->cross_term(dim, exp_inA, exp_inB, outC, bwL+1, bwL+1, bwL +1 + bwL +1);
  ...
  aux->multiplexer(drelu, res_exp, res_exp, dim, bwL + bwL+2, bwL + bwL+2);
  ...
  size_t comm_end = iopack->io->counter;
    ...
    for (int i = 0; i < dim; i++) {
        ...
        ideal_exp_plain[i] = std::exp(-fix2double(inA_original[i], inB_original[i], bwL_original, f_input));
    }
        double ulp_sum = 0.0;
    double ulp_max = 0.0;
  for (int i = 0; i < dim; i++) {
    ...
    //   }
    //   printf("res_exp[%d]: %llu\n", i, res_exp[i]);
    printf("inA_original[%d]: %llu\n", i, inA_original[i]);
    printf("inB_original[%d]: %llu\n", i, inB_original[i]);
    printf("inA[%d]: %llu\n", i, inA[i]);
    printf("inB[%d]: %llu\n", i, inB[i]);  
    printf("MW[%d]: %llu\n", i, MW[i]);
    printf("res_exp_plain[%d]: %f\n", i, res_exp_plain[i]);
